...
func Migrate(db *pgxpool.Pool) error {
	...
	migrations := []string{
		...
		`CREATE TABLE IF NOT EXISTS eros_runs (
			id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
			account_id UUID NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
			user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
			conversation_id UUID NOT NULL REFERENCES eros_conversations(id) ON DELETE CASCADE,
			kind TEXT NOT NULL CHECK (kind IN ('chat','quick_task')),
		...
		`ALTER TABLE eros_runs ADD COLUMN IF NOT EXISTS parent_run_id UUID REFERENCES eros_runs(id) ON DELETE SET NULL`,
		`DO $$ BEGIN
			ALTER TABLE eros_runs DROP CONSTRAINT IF EXISTS eros_runs_status_check;
			ALTER TABLE eros_runs ADD CONSTRAINT eros_runs_status_check CHECK (status IN ('queued','starting','running','waiting_for_input','completed','failed','cancelled'));
		EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
		`CREATE INDEX IF NOT EXISTS idx_eros_runs_queue ON eros_runs (status, next_attempt_at, created_at) WHERE status = 'queued'`,
		`CREATE INDEX IF NOT EXISTS idx_eros_runs_user_created ON eros_runs (account_id, user_id, created_at DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_eros_runs_conversation ON eros_runs (conversation_id, created_at DESC)`,
		`DO $$ BEGIN
			IF EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname=current_schema() AND indexname='uq_eros_runs_active_conversation' AND indexdef NOT ILIKE '%waiting_for_input%') THEN
				DROP INDEX uq_eros_runs_active_conversation;
			END IF;
		END $$`,
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_eros_runs_active_conversation ON eros_runs (conversation_id) WHERE status IN ('queued','starting','running','waiting_for_input')`,
		`CREATE INDEX IF NOT EXISTS idx_eros_runs_parent ON eros_runs (parent_run_id) WHERE parent_run_id IS NOT NULL`,
		`CREATE TABLE IF NOT EXISTS eros_result_sets (
			id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
			account_id UUID NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
			user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
			conversation_id UUID NOT NULL REFERENCES eros_conversations(id) ON DELETE CASCADE,
			run_id UUID NOT NULL REFERENCES eros_runs(id) ON DELETE CASCADE,
		...
		`CREATE INDEX IF NOT EXISTS idx_eros_result_sets_context ON eros_result_sets (account_id,user_id,conversation_id,created_at DESC)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_eros_result_sets_run_tool ON eros_result_sets (run_id,source_tool)`,
		`CREATE TABLE IF NOT EXISTS eros_result_set_items (
			result_set_id UUID NOT NULL REFERENCES eros_result_sets(id) ON DELETE CASCADE,
			account_id UUID NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
		...
		`CREATE INDEX IF NOT EXISTS idx_eros_result_set_items_account_entity ON eros_result_set_items (account_id,entity_id)`,
		`CREATE TABLE IF NOT EXISTS eros_context_grants (
			id UUID PRIMARY KEY,
			account_id UUID NOT NULL REFERENCES accounts(id) ON DELETE CASCADE,
			user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
		...
		`CREATE INDEX IF NOT EXISTS idx_eros_context_grants_expiry ON eros_context_grants (expires_at) WHERE revoked_at IS NULL`,
		...
		`ALTER TABLE eros_messages ADD COLUMN IF NOT EXISTS run_id UUID REFERENCES eros_runs(id) ON DELETE SET NULL`,
		`CREATE INDEX IF NOT EXISTS idx_eros_messages_conv ON eros_messages (conversation_id, created_at ASC)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_eros_messages_run_role ON eros_messages (run_id, role) WHERE run_id IS NOT NULL`,
		...
		`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_account_lead_pending_due ON tasks(account_id, lead_id, due_at) WHERE lead_id IS NOT NULL AND status='pending'`,
		`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_account_contact_pending_due ON tasks(account_id, contact_id, due_at) WHERE lead_id IS NULL AND contact_id IS NOT NULL AND status='pending'`,
	...
	if err := migrateTaskEvents(ctx, db); err != nil {
		return err
	}
