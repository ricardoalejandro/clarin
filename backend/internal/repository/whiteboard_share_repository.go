...
func (r *WhiteboardRepository) CreateShareLink(ctx context.Context, accountID, actorID, boardID uuid.UUID, input WhiteboardShareLinkInput) (*domain.WhiteboardShareLink, error) {
	...
	if _, err := requireWhiteboardAccessTx(ctx, tx, accountID, actorID, boardID, domain.WhiteboardAccessManage, true); err != nil {
		return nil, err
	}
...
func (r *WhiteboardRepository) CreateGuestSession(ctx context.Context, input WhiteboardGuestSessionInput) (*domain.WhiteboardGuestSession, error) {
	...
	if err := tx.QueryRow(ctx, `SELECT account_id,board_id,access_level,expires_at,max_sessions,session_count
		FROM whiteboard_share_links WHERE id=$1 AND revoked_at IS NULL
		AND (expires_at IS NULL OR expires_at>$2) FOR UPDATE`, input.LinkID, input.Now).Scan(
