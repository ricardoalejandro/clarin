...
- Excalidraw-powered Clarin whiteboards, editor integration or fork changes, documents/folders/revisions, whiteboard assets, import/export, sharing, collaboration, network isolation, licensing, or upstream upgrades: `.codex/skills/clarin-excalidraw-development/SKILL.md` plus every matching frontend, interface-design, backend, database, storage, and quality-assurance skill above.
...
- Put context/stage in the window header, render Contact identity once, and order the single-scroll detail as Contact information, tags, direct observations, general Contact history, contextual data, related tasks, then integrations. Direct observations and general Contact history stay separate but contiguous; the first three start open and the rest collapsed. Do not replace this hierarchy with Summary/Activity tabs or a competing activity rail.
...
- Lead detail changes stage through a viewport-aware portaled listbox limited to the current pipeline plus canonical `Sin etapa` (`stage_id: null`). It must reuse board mutation/reconciliation/rollback and existing won/lost/reopen confirmations; lost keeps its required reason.
...
- Archive Entornos, task folders, and lists only when every retained task/subtask is `done` or `cancelled`; preserve those closed tasks as read-only historical data. Folder archive records only the child lists archived by that operation, while Entorno archive preserves every child's own lifecycle. Unarchive must restore only operation-owned descendants. Concurrent structure/task mutations must lock and revalidate their dependencies inside one transaction.
- Completing a task is never deletion. Trash retention starts only from an explicit task, Entorno, list, or folder `deleted_at`; `archived_at` means historical Archive and must survive a Trash round trip. Never derive retention from done status, `completed_at`, progress, due dates, reports, or Archive. A container may enter Trash only when no retained task remains in its tree. Permanent purge is manual, account-admin-only, retention-gated, exact-name confirmed, and atomic across a deleted container tree.
