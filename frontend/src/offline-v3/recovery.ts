...
const recoveryKey = `clarin:offline-v3-recovery:${__CLARIN_OFFLINE_BUILD__}`
let handling = false

function isRuntimeAssetFailure(event: ErrorEvent | PromiseRejectionEvent) {
  if (event instanceof ErrorEvent && event.target instanceof HTMLScriptElement) return event.target.src.includes('/offline-v3/assets/')
...
function recover(event: ErrorEvent | PromiseRejectionEvent) {
  ...
  navigator.serviceWorker.controller?.postMessage({ type: 'CLARIN_OFFLINE_V3_USE_PREVIOUS' })
