...
export default function SurveyTemplatesPage() {
  ...
  const itemProps = { expandedId, instancesByTemplate, instancesLoading, onToggleInstances: toggleInstances, onDuplicate: setDuplicateTemplate, onArchive: requestArchive };
...
function TemplateCompact(props: ItemProps) {
  const { template } = props;
  return <article className="relative rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><h2 className="truncate font-semibold text-slate-900">{template.name}</h2><p className="mt-2 text-xs text-slate-500">{template.question_count} preguntas · {template.instance_count} aplicaciones · {template.response_count} respuestas</p></div><details className="relative"><summary className="flex min-h-11 min-w-11 cursor-pointer list-none items-center justify-center rounded-xl border border-slate-200 text-slate-500" aria-label={`Acciones de ${template.name}`}><Ellipsis className="h-4 w-4" /></summary><div className="absolute right-0 top-12 z-20 w-48 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl"><Link href={`/dashboard/surveys/${template.id}?mode=template`} className="flex min-h-10 items-center gap-2 rounded-lg px-3 text-sm text-slate-700 hover:bg-slate-50"><ChevronRight className="h-4 w-4" />Editar</Link><button type="button" onClick={() => props.onDuplicate(template)} className="flex min-h-10 w-full items-center gap-2 rounded-lg px-3 text-sm text-slate-700 hover:bg-slate-50"><Copy className="h-4 w-4" />Duplicar</button><button type="button" onClick={() => props.onArchive(template)} className="flex min-h-10 w-full items-center gap-2 rounded-lg px-3 text-sm text-slate-700 hover:bg-slate-50">{template.status === 'archived' ? <RotateCcw className="h-4 w-4" /> : <Archive className="h-4 w-4" />}{template.status === 'archived' ? 'Restaurar' : 'Archivar'}</button></div></details></div><button type="button" onClick={() => void props.onToggleInstances(template.id)} className="mt-3 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl bg-slate-100 text-xs font-semibold text-slate-700">Historial <ChevronDown className={`h-4 w-4 ${props.expandedId === template.id ? 'rotate-180' : ''}`} /></button><History {...props} inset /></article>;
}
...
function History(props: ItemProps & { inset?: boolean }) {
  ...
  return <div className={`${props.inset ? 'mt-3 rounded-xl border border-slate-100' : 'border-t border-slate-100'} bg-slate-50 p-3`}>{props.instancesLoading === template.id ? <div className="flex min-h-20 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-emerald-600" /></div> : instances.length === 0 ? <p className="p-4 text-center text-sm text-slate-500">Todavía no se ha aplicado esta plantilla.</p> : <div className="space-y-2">{instances.slice(0, 6).map(instance => <Link key={instance.id} href={`/dashboard/surveys/${instance.id}?mode=instance&tab=analytics`} className="flex min-h-14 items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 hover:border-emerald-200"><span className={`h-2.5 w-2.5 rounded-full ${instance.status === 'active' ? 'bg-emerald-500' : 'bg-slate-300'}`} /><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-slate-800">{instance.name}</span><span className="block truncate text-xs text-slate-500">{instance.origin_label} · {instance.response_count} respuestas</span></span><ChevronRight className="h-4 w-4 text-slate-400" /></Link>)}</div>}</div>;
}
