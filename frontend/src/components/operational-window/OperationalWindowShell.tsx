...
export interface OperationalWindowShellProps {
  open: boolean
  storageKey: string
  storageScope?: string
  title: string
  ...
  description?: string
  icon?: LucideIcon
  defaultMode?: Exclude<OperationalWindowMode, 'maximized'>
  defaultWidth?: number
...
export default function OperationalWindowShell({
  ...
  dataAttribute = 'operational-window',
  overlayZIndex = 120,
    ...
    >
      ...
      >
        ...
        <header
          ...
          onDoubleClick={event => { if (!(event.target as HTMLElement).closest('button,a,input,textarea,select,[data-no-window-drag]')) windowState.toggleMaximized() }}
          className={`relative z-20 flex shrink-0 select-none items-start gap-3 border-b border-slate-100 bg-white px-4 py-3 sm:px-6 sm:py-4 ${windowState.effectiveMode === 'floating' ? 'cursor-move' : ''}`}
        >
          ...
          <div data-no-window-drag className="flex shrink-0 items-center gap-1">
            {headerActions}
            {!windowState.isMobile && !windowState.temporaryModeActive && <>
              ...
              {windowState.effectiveMode === 'floating' && <button type="button" title="Restablecer tamaño" aria-label="Restablecer tamaño" onClick={windowState.resetGeometry} className="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><RotateCcw className="h-4 w-4" /></button>}
