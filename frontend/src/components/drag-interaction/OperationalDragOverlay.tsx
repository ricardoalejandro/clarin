...
export default function OperationalDragOverlay({ label, count = 1, singular, plural, destination, destinationColor, sourceWidth }: Props) {
  const layers = operationalDragStackLayers(count)
  const width = operationalDragOverlayWidth(sourceWidth)
  return (
    <div data-operational-drag-overlay className="relative h-[92px] max-w-full motion-reduce:transition-none" style={{ width }} aria-label={`${count} ${count === 1 ? singular : plural} seleccionad${count === 1 ? 'o' : 'os'}`}>
