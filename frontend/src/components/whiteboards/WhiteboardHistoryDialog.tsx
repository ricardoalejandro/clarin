...
function versionLabel(kind: string) {
  if (kind === 'restore') return 'Restauración'
  if (kind === 'patch') return 'Guardado automático'
...
export default function WhiteboardHistoryDialog({
  ...
  return <WhiteboardModal title="Historial de versiones" description="Cada restauración crea una nueva versión; ninguna versión anterior se sobrescribe." onClose={onClose} wide>
    ...
    {phase === 'ready' && <div className="p-4 sm:p-5">
      ...
      {!versions.length ? <div className="flex min-h-52 flex-col items-center justify-center text-center"><Clock3 className="h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-700">Aún no hay versiones guardadas.</p></div> : <div className="space-y-2">{versions.map(version => <article key={version.id} className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 p-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-500"><Clock3 className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="text-sm font-black text-slate-800">Versión {version.revision_number}</p><p className="mt-0.5 text-xs text-slate-500">{versionLabel(version.write_kind)} · {new Intl.DateTimeFormat('es', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(version.created_at))}</p></div><div className="flex gap-2"><button type="button" onClick={() => void openPreview(version)} disabled={Boolean(previewBusyID)} className="flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 px-3 text-xs font-black text-slate-600 hover:bg-slate-50 disabled:opacity-35">{previewBusyID === version.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Eye className="h-4 w-4" />}Ver</button><button type="button" onClick={() => void restore(version)} disabled={!canRestore || Boolean(busyID)} title={!canRestore ? 'Necesitas permiso de edición para restaurar.' : undefined} className="flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 px-3 text-xs font-black text-slate-600 hover:bg-slate-50 disabled:opacity-35">{busyID === version.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <RotateCcw className="h-4 w-4" />}Restaurar</button></div></article>)}</div>}
