...
export type TaskListScopeType = 'all' | 'environment' | 'folder' | 'list' | 'shared' | 'archive' | 'trash'

export function taskListShouldShowLocation(scopeType: TaskListScopeType, groupBy: TaskGroupBy) {
  return scopeType !== 'list' && groupBy !== 'list'
}

export function taskListRowMetadata(task: Task, density: ReturnType<typeof taskListDensity>, showLocation: boolean, editable: boolean) {
  const values: string[] = []
  if (task.breadcrumbs_visible === false) values.push('Compartida contigo')
  else if (showLocation) values.push(taskLocationLabel(task))
  if (density === 'stacked') {
    values.push(
      task.status_detail?.name || 'Sin estado',
      task.assigned_to_name || 'Sin responsable',
      task.due_at ? new Date(task.due_at).toLocaleDateString('es', { day: 'numeric', month: 'short' }) : 'Sin fecha',
    )
  }
  if (!editable) values.push('Solo lectura')
  return values.join(' · ')
}
...
function TaskRowRenameField({ task, metadata, rename }: {
  ...
  return <div className="min-w-0 flex-1" onPointerDown={event => event.stopPropagation()} onClick={event => event.stopPropagation()}>
    ...
    {(rename.error || metadata) && <span role={rename.error ? 'alert' : undefined} className={`block truncate text-[10px] leading-4 ${rename.error ? 'font-semibold text-rose-600' : 'text-slate-400'}`}>{rename.error || metadata}</span>}
  </div>
}

function TaskRowActions({ task, editable, pinned, rename, density, onAddSubtask, onStar }: {
  task: Task
  editable: boolean
  pinned: boolean
  rename: TaskRowRenameController
  ...
  const directActions = presentation.actionMode === 'direct'
  const actionSlotClass = directActions && !onAddSubtask
    ? 'w-[70px] [@media(pointer:coarse)]:w-11'
    : presentation.actionSlotClass
  const progressiveClass = taskRowProgressiveControlClass(pinned || touchMenuOpen)
  return <div
    data-task-row-actions
    data-task-row-action-mode={presentation.actionMode}
    data-task-row-controls-pinned={pinned || undefined}
    className={`relative flex h-11 shrink-0 items-center justify-end gap-0.5 ${actionSlotClass}`}
    onPointerDown={event => event.stopPropagation()}
    onClick={event => event.stopPropagation()}
  >
    {directActions && onAddSubtask && <button
      type="button"
      ...
      className={`flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 outline-none transition hover:bg-emerald-50 hover:text-emerald-700 focus-visible:ring-2 focus-visible:ring-emerald-400 disabled:cursor-not-allowed disabled:opacity-35 [@media(pointer:coarse)]:hidden ${progressiveClass}`}
    ><Plus className="h-4 w-4" /></button>}
    {directActions && <button
      type="button"
      disabled={rename.pending}
      ...
      className={`flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 outline-none transition hover:bg-slate-100 hover:text-slate-800 focus-visible:ring-2 focus-visible:ring-emerald-400 disabled:cursor-wait disabled:opacity-45 [@media(pointer:coarse)]:hidden ${progressiveClass}`}
    ...
    {directActions && onStar && <button
      type="button"
      disabled={rename.pending}
      aria-label={task.starred ? `Quitar ${task.title} de destacadas` : `Destacar ${task.title}`}
      title={task.starred ? 'Quitar de destacadas' : 'Destacar tarea'}
      onPointerDown={event => event.stopPropagation()}
      onClick={event => { event.stopPropagation(); onStar() }}
      className={`flex h-8 w-8 items-center justify-center rounded-lg outline-none transition hover:bg-amber-50 focus-visible:ring-2 focus-visible:ring-emerald-400 disabled:cursor-wait disabled:opacity-45 [@media(pointer:coarse)]:hidden ${task.starred ? 'pointer-events-auto text-amber-500 opacity-100' : `text-slate-400 ${progressiveClass}`}`}
    ><Star className={`h-4 w-4 ${task.starred ? 'fill-current' : ''}`} /></button>}
    <button
      ref={touchTriggerRef}
      type="button"
      disabled={rename.editing || rename.pending}
      aria-label={`Acciones de ${task.title}${task.starred ? ', destacada' : ''}`}
      aria-haspopup="menu"
      aria-expanded={touchMenuOpen}
      title="Acciones de la tarea"
      onPointerDown={event => event.stopPropagation()}
      onClick={event => { event.stopPropagation(); setTouchMenuOpen(value => !value) }}
      className={`${directActions ? 'hidden' : 'flex'} relative h-9 w-9 items-center justify-center rounded-lg outline-none transition hover:bg-slate-100 focus-visible:ring-2 focus-visible:ring-emerald-400 disabled:opacity-40 [@media(pointer:coarse)]:flex [@media(pointer:coarse)]:h-11 [@media(pointer:coarse)]:w-11 ${task.starred ? 'pointer-events-auto text-slate-600 opacity-100' : `text-slate-600 ${progressiveClass}`}`}
    ><MoreHorizontal className="h-5 w-5" />{task.starred && <span data-task-starred-indicator aria-hidden="true" className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-amber-400 ring-2 ring-white" />}</button>
...
function SortableRow({ task, groupKey, active, selected, selectionMode, density, showLocation, allStatuses, subtaskExpanded, onToggleSubtasks, onOpen, onSelect, onStatus, onStar, onAddSubtask, onRenameTask }: {
  task: Task; groupKey: string; active: boolean; selected: boolean; selectionMode: boolean; density: ReturnType<typeof taskListDensity>; showLocation: boolean; allStatuses: TaskWorkflowStatus[]; subtaskExpanded: boolean; onToggleSubtasks: () => void; onOpen: (trigger?: HTMLElement | null) => void; onSelect: (shift: boolean) => void; onStatus: (statusId: string) => void; onStar: () => void; onAddSubtask: (trigger: HTMLButtonElement) => void; onRenameTask: Props['onRenameTask']
}) {
  ...
  const metadata = taskListRowMetadata(task, density, showLocation, editable)
  const presentation = taskListPresentation(density)
...
function SubtaskRow({ task, allStatuses, density, active, pending, onOpen, onStatus, onStar, onRenameTask }: {
  task: Task
  ...
  const progressiveControlClass = taskRowProgressiveControlClass(controlsPinned)
  return <div data-task-subtask-row={task.id} data-task-active={active || undefined} data-task-row-pending={(pending || rename.pending) || undefined} aria-busy={(pending || rename.pending) || undefined} aria-current={active ? 'true' : undefined} className={`group relative grid min-h-11 items-center gap-1.5 rounded-lg px-2.5 transition hover:bg-white focus-within:bg-white [@media(pointer:coarse)]:min-h-14 [@media(pointer:coarse)]:gap-2 [@media(pointer:coarse)]:px-3 ${active ? 'before:absolute before:inset-y-2 before:right-0.5 before:w-0.5 before:rounded-full before:bg-slate-500' : ''} ${presentation.gridClass}`}>
...
export default function TaskListView({ tasks, statuses, lists, folders, users, groupBy, groupDirection, collapsedGroupKeys, scopeType, subtaskDisplayMode, subtaskScope, subtaskRealtimeEvent, activeTaskId, onGroupingChange, onOpen, onStatus, onStar, onAddSubtask, onRenameTask, onCanonicalTasks, onHierarchyCounts, onOperation, onDragStateChange, onExternalDropTargetChange, onRefresh, onError }: Props) {
  ...
  const showLocation = taskListShouldShowLocation(scopeType, groupBy)
