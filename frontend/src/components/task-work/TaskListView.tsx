...
function SortableRow({ task, groupKey, selected, selectionMode, density, allStatuses, onOpen, onSelect, onStatus, onStar }: {
  ...
  const compactMetadata = density === 'stacked'
    ...
    : ''
  const metadata = `${taskLocationLabel(task)}${task.subtask_count ? ` · ${task.subtask_done}/${task.subtask_count} subtareas` : ''}${compactMetadata}${!editable ? ' · Solo lectura' : ''}`
  ...
  return <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, backgroundColor: selected ? taskIdentityTint(identityColor, .1) : undefined }} data-task-list-row={task.id} onClick={event => {
    ...
    <button type="button" disabled={!editable} aria-label={task.starred ? `Quitar ${task.title} de destacadas` : `Destacar ${task.title}`} onClick={event => { event.stopPropagation(); onStar() }} className={`flex h-11 w-full items-center justify-center rounded-lg outline-none focus:ring-2 focus:ring-emerald-400 disabled:cursor-not-allowed disabled:opacity-25 ${task.starred ? 'text-amber-400' : 'text-slate-300 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 [@media(pointer:coarse)]:opacity-100'}`}><Star className={`h-4 w-4 ${task.starred ? 'fill-current' : ''}`} /></button>
  </div>
}
