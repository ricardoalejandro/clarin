...
function SortableRow({ task, groupKey, selected, selectionMode, density, allStatuses, onOpen, onSelect, onStatus, onStar }: {
  ...
  return <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} data-task-list-row={task.id} onClick={event => {
    ...
    <button type="button" {...attributes} {...listeners} disabled={!editable} onClick={event => event.stopPropagation()} aria-label={editable ? `Arrastrar ${task.title}` : `${task.title}: solo lectura`} title={editable ? 'Arrastrar tarea' : 'No tienes permiso para mover esta tarea'} className="flex h-8 w-7 cursor-grab items-center justify-center rounded-lg text-slate-300 outline-none transition hover:bg-slate-100 hover:text-slate-600 focus:ring-2 focus:ring-emerald-400 active:cursor-grabbing disabled:cursor-default disabled:opacity-25"><GripVertical className="h-4 w-4" /></button>
    <div className="flex min-w-0 items-center gap-3"><button type="button" disabled={!editable} onClick={event => { event.stopPropagation(); const next = allowed.find(status => status.category === (done ? 'not_started' : 'done')); if (next) onStatus(next.id) }} className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border disabled:cursor-not-allowed disabled:opacity-55 ${done ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-slate-300 bg-white'}`}>{done && <Check className="h-3 w-3" />}</button><div className="min-w-0"><p className={`truncate text-sm font-semibold ${done ? 'text-slate-400 line-through' : 'text-slate-700'}`}>{task.title}</p><p className="mt-0.5 truncate text-[10px] text-slate-400">{taskLocationLabel(task)}{task.subtask_count ? ` · ${task.subtask_done}/${task.subtask_count} subtareas` : ''}{density === 'stacked' ? ` · ${task.assigned_to_name || 'Sin responsable'}` : ''}{!editable ? ' · Solo lectura' : ''}</p></div>{selected && <span className="ml-auto rounded-full bg-emerald-600 px-2 py-0.5 text-[9px] font-black text-white">Seleccionada</span>}</div>
    {density !== 'stacked' && <><div onClick={event => event.stopPropagation()}><TaskStatusPicker value={task.status_id || ''} statuses={allowed} compact={density === 'compact'} disabled={!editable} onChange={onStatus} /></div><div className="flex min-w-0 items-center gap-2"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 text-[9px] font-black text-slate-500">{(task.assigned_to_name || '?').slice(0,2).toUpperCase()}</span><span className="truncate text-xs text-slate-500">{task.assigned_to_name || 'Sin responsable'}</span></div><span className="text-xs text-slate-400">{task.due_at ? new Date(task.due_at).toLocaleDateString('es', { day: 'numeric', month: 'short' }) : 'Sin fecha'}</span><button type="button" disabled={!editable} onClick={event => { event.stopPropagation(); onStar() }} className={`rounded-lg p-2 disabled:cursor-not-allowed disabled:opacity-25 ${task.starred ? 'text-amber-400' : 'text-slate-300 opacity-0 group-hover:opacity-100'}`}><Star className={`h-4 w-4 ${task.starred ? 'fill-current' : ''}`} /></button></>}
  </div>
}
