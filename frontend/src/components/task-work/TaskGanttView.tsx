...
import { Diamond, Link2, ZoomIn, ZoomOut } from 'lucide-react'
...
export default function TaskGanttView({ data, onOpen, onMove }: Props) {
  ...
  const drag = (event: React.PointerEvent, task: Task, mode: 'move' | 'start' | 'end') => {
    ...
    const preview = (pointer: PointerEvent) => {
      const delta = Math.round((pointer.clientX - initialX) / cellWidth)
    ...
    <div className="min-h-0 flex-1 overflow-auto">
      <div className="relative" style={{ width: LABEL + dayCount * cellWidth, minHeight: 44 + tasks.length * ROW }}>
        ...
        <div className="relative z-20">
          {tasks.map((task, index) => { let { start, due } = datesFor(task); if (dragPreview?.taskId === task.id) { if (dragPreview.mode === 'move' || dragPreview.mode === 'start') start = addDays(start, dragPreview.delta); if (dragPreview.mode === 'move' || dragPreview.mode === 'end') due = addDays(due, dragPreview.delta) } const left = ((startOfDay(start).getTime() - minDate.getTime()) / DAY) * cellWidth; const duration = Math.max(1, Math.ceil((due.getTime() - start.getTime()) / DAY)); const width = Math.max(cellWidth, duration * cellWidth); const isCritical = critical.has(task.id); const isDone = task.status_detail?.category === 'done'; const invalidPreview = due < start; return <div key={task.id} className="relative flex border-b border-slate-100" style={{ height: ROW }}>
