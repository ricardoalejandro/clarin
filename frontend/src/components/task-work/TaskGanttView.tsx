        ...
        <div className="relative z-20">
          {tasks.map((task, index) => { let { start, due } = datesFor(task); if (dragPreview?.taskId === task.id) { if (dragPreview.mode === 'move' || dragPreview.mode === 'start') start = addDays(start, dragPreview.delta); if (dragPreview.mode === 'move' || dragPreview.mode === 'end') due = addDays(due, dragPreview.delta) } const left = ((startOfDay(start).getTime() - minDate.getTime()) / DAY) * effectiveCellWidth; const duration = Math.max(1, Math.ceil((due.getTime() - start.getTime()) / DAY)); const width = Math.max(effectiveCellWidth, duration * effectiveCellWidth); const isCritical = critical.has(task.id); const isDone = task.status_detail?.category === 'done'; const invalidPreview = due < start; return <div key={task.id} className="relative flex border-b border-slate-100" style={{ height: ROW }}>
            ...
            <div className="relative" style={{ width: dayCount * effectiveCellWidth }}>
              {task.is_milestone ? <button onClick={() => openTask(task)} onPointerDown={event => drag(event, task, 'move')} className="absolute top-1/2 z-20 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rotate-45 rounded-sm bg-violet-500 shadow-md" style={{ left }} title={task.title} /> : <div onClick={() => openTask(task)} onPointerDown={event => drag(event, task, 'move')} className={`group absolute top-2.5 z-20 h-6 cursor-grab overflow-hidden rounded-md shadow-sm transition ${moving === task.id ? `opacity-75 ring-2 ${invalidPreview ? 'ring-rose-300' : 'ring-emerald-300'}` : 'hover:-translate-y-px hover:shadow-md'} ${isCritical || invalidPreview ? 'bg-rose-500' : isDone ? 'bg-emerald-500' : 'bg-slate-700'}`} style={{ left, width }}>
                <div className="absolute inset-y-0 left-0 bg-white/20" style={{ width: `${task.progress || 0}%` }} />
                <button onPointerDown={event => drag(event, task, 'start')} className="absolute inset-y-0 left-0 z-30 w-2.5 cursor-ew-resize border-l-2 border-white/80 bg-white/15 hover:bg-white/35" aria-label="Cambiar inicio" />
                <span className="pointer-events-none relative block truncate px-3 py-1 text-[10px] font-semibold text-white">{task.title}</span>
                <button onPointerDown={event => drag(event, task, 'end')} className="absolute inset-y-0 right-0 z-30 w-2.5 cursor-ew-resize border-r-2 border-white/80 bg-white/15 hover:bg-white/35" aria-label="Cambiar entrega" />
