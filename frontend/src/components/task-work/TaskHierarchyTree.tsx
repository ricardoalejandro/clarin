...
function SortableListRow({ list, containerID, active, selected, onSelect, onEdit }: {
  ...
  return <div ref={sortable.setNodeRef} style={{ transform: CSS.Transform.toString(sortable.transform), transition: sortable.transition }} data-task-hierarchy-list={list.id} data-task-drop-list={list.id} data-task-drop-label={list.name} className={`group flex items-center rounded-lg transition ${active ? 'opacity-30' : ''} ${selected ? 'bg-emerald-50 text-emerald-700' : 'text-slate-500 hover:bg-slate-50'}`}>
...
function SortableFolderBlock({ folder, items, activeFolderID, activeListID, highlighted, expanded, scope, onSelect, onToggle, onEditFolder, onEditList }: {
  ...
  return <div ref={sortable.setNodeRef} style={{ transform: CSS.Transform.toString(sortable.transform), transition: sortable.transition }} data-task-drop-folder={folder.id} data-task-drop-label={folder.name} className={activeFolderID === folder.id ? 'opacity-30' : ''}>
...
export default function TaskHierarchyTree({ folders, rootLists, scope, collapsed, onSelect, onChanged, onError, onOperation }: Props) {
  ...
  if (collapsed) return <div className="space-y-1.5 px-1">
    {defaultList && <button type="button" title={`${defaultList.name} · ${defaultList.task_count} tareas`} onClick={() => onSelect({ type: 'list', id: defaultList.id })} className={`relative flex h-10 w-full items-center justify-center rounded-xl border transition ${scope.type === 'list' && scope.id === defaultList.id ? 'border-emerald-200 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-transparent text-slate-500 hover:border-slate-200 hover:bg-slate-50'}`}><TaskContainerIcon value={defaultList.icon || 'inbox'} className="h-4 w-4" />{Boolean(defaultList.task_count) && <span className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-slate-900 px-1 text-[8px] font-black leading-4 text-white">{defaultList.task_count}</span>}</button>}
    {(ordered.root || []).map(list => <button key={list.id} type="button" title={`${list.name} · ${list.task_count} tareas`} onClick={() => onSelect({ type: 'list', id: list.id })} className={`relative flex h-10 w-full items-center justify-center rounded-xl border transition ${scope.type === 'list' && scope.id === list.id ? 'border-emerald-200 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-transparent text-slate-500 hover:border-slate-200 hover:bg-slate-50'}`}><TaskContainerIcon value={list.icon} className="h-4 w-4" />{Boolean(list.task_count) && <span className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-slate-900 px-1 text-[8px] font-black leading-4 text-white">{list.task_count}</span>}</button>)}
    <div className="mx-auto my-2 h-px w-7 bg-slate-200" />
    {orderedFolders.map(folder => <button key={folder.id} type="button" title={`${folder.name} · ${folder.task_count} tareas`} onClick={() => onSelect({ type: 'folder', id: folder.id })} className={`relative flex h-10 w-full items-center justify-center rounded-xl border transition ${scope.type === 'folder' && scope.id === folder.id ? 'border-emerald-200 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-transparent text-slate-500 hover:border-slate-200 hover:bg-slate-50'}`}><TaskContainerIcon value={folder.icon} className="h-4 w-4" />{Boolean(folder.task_count) && <span className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-slate-900 px-1 text-[8px] font-black leading-4 text-white">{folder.task_count}</span>}</button>)}
  </div>
