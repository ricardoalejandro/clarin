...
import { AlertCircle, Check, ChevronRight, FolderOpen, GripVertical, Info, Loader2, LockKeyhole, MoreHorizontal, Plus, RotateCcw, Undo2 } from 'lucide-react'
...
function DefaultListRow({ list, selected, taskDropActive, onSelect, onEdit }: { list: TaskList; selected: boolean; taskDropActive: boolean; onSelect: () => void; onEdit: () => void }) {
  ...
  const canReceiveTasks = taskHierarchyCanReceiveTasks(list)
  const dropActive = canReceiveTasks && taskDropActive
  return <div data-task-default-list data-task-drop-list={canReceiveTasks ? list.id : undefined} data-task-drop-label={canReceiveTasks ? list.name : undefined} data-task-drop-color={canReceiveTasks ? list.color : undefined} data-task-drop-highlight={dropActive || undefined} className={`group relative flex min-h-10 items-center rounded-xl transition-all duration-150 [@media(pointer:coarse)]:min-h-11 ${dropActive ? 'z-10 scale-[1.02] bg-emerald-50 text-emerald-800 shadow-[0_8px_22px_rgba(16,185,129,0.18)] ring-2 ring-emerald-400' : selected ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50'}`}><button type="button" onClick={onSelect} title={hierarchyCountTooltip(list)} className="flex min-h-10 min-w-0 flex-1 items-center gap-2 px-2 text-left text-xs font-semibold [@media(pointer:coarse)]:min-h-11"><span className="flex h-6 w-6 items-center justify-center rounded-lg" style={{ color: list.color, backgroundColor: `${list.color}16` }}><TaskContainerIcon value={list.icon || 'inbox'} className="h-3.5 w-3.5" /></span><span className="min-w-0 flex-1 truncate">{list.name}</span><span aria-label={`${list.open_task_count || 0} tareas abiertas`} className="text-[9px] text-slate-400">{list.open_task_count || 0}</span></button>{dropActive && <span className="pointer-events-none absolute right-1 top-full z-20 mt-1 whitespace-nowrap rounded-full bg-emerald-700 px-2 py-1 text-[9px] font-black text-white shadow-lg">Soltar en {list.name}</span>}{canManage && <button type="button" onClick={onEdit} aria-label={`Opciones de la lista ${list.name}`} title="Opciones de la lista" className={`flex h-8 w-8 items-center justify-center rounded-md text-slate-300 hover:bg-white hover:text-slate-600 focus:opacity-100 group-hover:opacity-100 [@media(pointer:coarse)]:opacity-100 ${selected ? 'opacity-100' : 'opacity-0'}`}><MoreHorizontal className="h-3.5 w-3.5" /></button>}<span title="La Bandeja general permanece fija en la raíz" className="mr-1 flex h-8 w-8 items-center justify-center text-slate-300"><LockKeyhole className="h-3 w-3" /></span></div>
}
...
function SortableFolderBlock({ folder, items, activeFolderID, activeListID, highlighted, taskDropTarget, expanded, scope, childrenState, onSelect, onToggle, onRetryChildren, onLoadMoreChildren, onEditFolder, onEditList }: {
  ...
  return <div ref={sortable.setNodeRef} style={{ transform: CSS.Transform.toString(sortable.transform), transition: sortable.transition }} data-task-drop-folder={canReceiveTasks ? folder.id : undefined} data-task-drop-label={canReceiveTasks ? folder.name : undefined} data-task-drop-color={canReceiveTasks ? folder.color : undefined} className={activeFolderID === folder.id ? 'opacity-30' : ''}>
    <ListDropContainer id={containerID} active={canManage && highlighted} label={`Carpeta ${folder.name}`} disabled={!canManage}>
      <div data-task-drop-highlight={taskDropActive || undefined} className={`group relative flex min-h-11 items-center rounded-xl transition-all duration-150 ${taskDropActive ? 'z-10 scale-[1.02] bg-emerald-50 shadow-[0_8px_22px_rgba(16,185,129,0.18)] ring-2 ring-emerald-400' : ''}`}><button type="button" onClick={onToggle} aria-label={`${expanded ? 'Contraer' : 'Expandir'} ${folder.name}`} aria-expanded={expanded} className="ml-0.5 flex h-11 w-9 shrink-0 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"><ChevronRight className={`h-3.5 w-3.5 transition-transform duration-200 ${expanded ? 'rotate-90' : ''}`} /></button><button type="button" onClick={() => onSelect({ type: 'folder', id: folder.id })} title={hierarchyCountTooltip(folder)} className={`flex min-h-11 min-w-0 flex-1 items-center gap-2 rounded-xl px-1 text-sm font-medium ${taskDropActive ? 'text-emerald-800' : selected ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:bg-slate-50'}`}><span className="flex h-7 w-7 items-center justify-center rounded-lg" style={{ color: folder.color, backgroundColor: `${folder.color}16` }}>{taskDropActive || (canManage && highlighted) || expanded ? <FolderOpen className="h-4 w-4" /> : <TaskContainerIcon value={folder.icon} className="h-4 w-4" />}</span><span className="min-w-0 flex-1 truncate text-left">{folder.name}</span><span aria-label={`${folder.open_task_count || 0} tareas abiertas`} className="text-[10px] text-slate-400">{folder.open_task_count || 0}</span></button>{taskDropActive && <span className="pointer-events-none absolute right-1 top-full z-20 mt-1 whitespace-nowrap rounded-full bg-emerald-700 px-2 py-1 text-[9px] font-black text-white shadow-lg">Suelta para elegir una lista</span>}{canManage && <button type="button" onClick={onEditFolder} aria-label={`Personalizar ${folder.name}`} className="flex h-9 w-9 items-center justify-center rounded-md text-slate-300 opacity-0 hover:bg-white hover:text-slate-600 focus:opacity-100 group-hover:opacity-100"><MoreHorizontal className="h-3.5 w-3.5" /></button>}{canManage && <button type="button" ref={sortable.setActivatorNodeRef} {...sortable.attributes} {...sortable.listeners} aria-label={`Mover carpeta ${folder.name}`} className="mr-0.5 flex h-9 w-9 cursor-grab items-center justify-center rounded-md text-slate-300 opacity-0 hover:bg-white hover:text-slate-600 focus:opacity-100 active:cursor-grabbing group-hover:opacity-100"><GripVertical className="h-3.5 w-3.5" /></button>}</div>
...
export default function TaskHierarchyTree({ folders, rootLists, scope, collapsed, users = [], onSelect, onChanged, onError, onOperation, taskDropTarget, taskDragActive = false, hierarchyPhase = 'ready', hierarchyError = '', hasMoreFolders = false, hasMoreRootLists = false, loadingMoreFolders = false, loadingMoreRootLists = false, folderChildrenState = {}, onRetryHierarchy, onLoadMoreFolders, onLoadMoreRootLists, onExpandFolder, onRetryFolderLists, onLoadMoreFolderLists, onOpenStructure }: Props) {
  ...
  const independentHeadingVisible = showIndependentListsHeading(
    (ordered.root || []).length + Number(hasMoreRootLists),
    hierarchyPhase,
    Boolean(activeListID || overContainerID === 'root'),
  )
  ...
  if (collapsed) return <div className="space-y-1.5 px-1">
    {hierarchyPhase === 'loading' && !rootLists.length && !folders.length && <div role="status" aria-label="Cargando jerarquía" className="flex h-11 items-center justify-center text-slate-400"><Loader2 className="h-4 w-4 animate-spin" /></div>}
    {hierarchyPhase === 'error' && !rootLists.length && !folders.length && <button type="button" aria-label="Reintentar jerarquía" title={hierarchyError} onClick={onRetryHierarchy} className="flex h-11 w-full items-center justify-center rounded-xl bg-rose-50 text-rose-600"><AlertCircle className="h-4 w-4" /></button>}
    {defaultList && <button type="button" data-task-drop-list={taskHierarchyCanReceiveTasks(defaultList) ? defaultList.id : undefined} data-task-drop-label={taskHierarchyCanReceiveTasks(defaultList) ? defaultList.name : undefined} data-task-drop-color={taskHierarchyCanReceiveTasks(defaultList) ? defaultList.color : undefined} title={`${defaultList.name} · ${hierarchyCountTooltip(defaultList)}`} onClick={() => onSelect({ type: 'list', id: defaultList.id })} className={`relative flex h-11 w-full items-center justify-center rounded-xl border transition ${taskHierarchyCanReceiveTasks(defaultList) && taskDropTarget?.type === 'list' && taskDropTarget.id === defaultList.id ? 'scale-105 border-emerald-400 bg-emerald-50 text-emerald-700 shadow-lg ring-2 ring-emerald-200' : scope.type === 'list' && scope.id === defaultList.id ? 'border-emerald-200 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-transparent text-slate-500 hover:border-slate-200 hover:bg-slate-50'}`}><TaskContainerIcon value={defaultList.icon || 'inbox'} className="h-4 w-4" />{Boolean(hierarchyItemOpenCount(defaultList)) && <span aria-label={`${hierarchyItemOpenCount(defaultList)} tareas abiertas`} className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-slate-900 px-1 text-[8px] font-black leading-4 text-white">{hierarchyItemOpenCount(defaultList)}</span>}</button>}
    {(ordered.root || []).map(list => <button key={list.id} type="button" data-task-drop-list={taskHierarchyCanReceiveTasks(list) ? list.id : undefined} data-task-drop-label={taskHierarchyCanReceiveTasks(list) ? list.name : undefined} data-task-drop-color={taskHierarchyCanReceiveTasks(list) ? list.color : undefined} title={`${list.name} · ${hierarchyCountTooltip(list)}`} onClick={() => onSelect({ type: 'list', id: list.id })} className={`relative flex h-11 w-full items-center justify-center rounded-xl border transition ${taskHierarchyCanReceiveTasks(list) && taskDropTarget?.type === 'list' && taskDropTarget.id === list.id ? 'scale-105 border-emerald-400 bg-emerald-50 text-emerald-700 shadow-lg ring-2 ring-emerald-200' : scope.type === 'list' && scope.id === list.id ? 'border-emerald-200 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-transparent text-slate-500 hover:border-slate-200 hover:bg-slate-50'}`}><TaskContainerIcon value={list.icon} className="h-4 w-4" />{Boolean(hierarchyItemOpenCount(list)) && <span aria-label={`${hierarchyItemOpenCount(list)} tareas abiertas`} className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-slate-900 px-1 text-[8px] font-black leading-4 text-white">{hierarchyItemOpenCount(list)}</span>}</button>)}
    <div className="mx-auto my-2 h-px w-7 bg-slate-200" />
    {orderedFolders.map(folder => <button key={folder.id} type="button" data-task-drop-folder={taskHierarchyCanReceiveTasks(folder) ? folder.id : undefined} data-task-drop-label={taskHierarchyCanReceiveTasks(folder) ? folder.name : undefined} data-task-drop-color={taskHierarchyCanReceiveTasks(folder) ? folder.color : undefined} title={`${folder.name} · ${hierarchyCountTooltip(folder)}`} onClick={() => onSelect({ type: 'folder', id: folder.id })} className={`relative flex h-11 w-full items-center justify-center rounded-xl border transition ${taskHierarchyCanReceiveTasks(folder) && taskDropTarget?.type === 'folder' && taskDropTarget.id === folder.id ? 'scale-105 border-emerald-400 bg-emerald-50 text-emerald-700 shadow-lg ring-2 ring-emerald-200' : scope.type === 'folder' && scope.id === folder.id ? 'border-emerald-200 bg-emerald-50 text-emerald-700 shadow-sm' : 'border-transparent text-slate-500 hover:border-slate-200 hover:bg-slate-50'}`}>{taskHierarchyCanReceiveTasks(folder) && taskDropTarget?.type === 'folder' && taskDropTarget.id === folder.id ? <FolderOpen className="h-4 w-4" /> : <TaskContainerIcon value={folder.icon} className="h-4 w-4" />}{Boolean(hierarchyItemOpenCount(folder)) && <span aria-label={`${hierarchyItemOpenCount(folder)} tareas abiertas`} className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-slate-900 px-1 text-[8px] font-black leading-4 text-white">{hierarchyItemOpenCount(folder)}</span>}</button>)}
  </div>
