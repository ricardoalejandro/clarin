...
function DefaultListRow({ list, selected, taskDropActive, onSelect, onEdit }: { list: TaskList; selected: boolean; taskDropActive: boolean; onSelect: () => void; onEdit: () => void }) {
  ...
  const canReceiveTasks = taskHierarchyCanReceiveTasks(list)
  const dropActive = canReceiveTasks && taskDropActive
  return <div data-task-default-list data-task-drop-list={canReceiveTasks ? list.id : undefined} data-task-drop-label={canReceiveTasks ? list.name : undefined} data-task-drop-color={canReceiveTasks ? list.color : undefined} data-task-drop-highlight={dropActive || undefined} className={`group relative flex min-h-11 items-center rounded-xl transition-all duration-150 ${dropActive ? 'z-10 scale-[1.02] bg-emerald-50 text-emerald-800 shadow-[0_8px_22px_rgba(16,185,129,0.18)] ring-2 ring-emerald-400' : selected ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50'}`}><button type="button" onClick={onSelect} title={hierarchyCountTooltip(list)} className="flex min-h-11 min-w-0 flex-1 items-center gap-2 px-2 text-left text-xs font-semibold"><span className="flex h-6 w-6 items-center justify-center rounded-lg" style={{ color: list.color, backgroundColor: `${list.color}16` }}><TaskContainerIcon value={list.icon || 'inbox'} className="h-3.5 w-3.5" /></span><span className="min-w-0 flex-1 truncate">{list.name}</span><span aria-label={`${list.open_task_count || 0} tareas abiertas`} className="text-[9px] text-slate-400">{list.open_task_count || 0}</span></button>{dropActive && <span className="pointer-events-none absolute right-1 top-full z-20 mt-1 whitespace-nowrap rounded-full bg-emerald-700 px-2 py-1 text-[9px] font-black text-white shadow-lg">Soltar en {list.name}</span>}{canManage && <button type="button" onClick={onEdit} aria-label={`Personalizar ${list.name}`} className="flex h-9 w-9 items-center justify-center rounded-md text-slate-300 opacity-0 hover:bg-white hover:text-slate-600 focus:opacity-100 group-hover:opacity-100"><MoreHorizontal className="h-3.5 w-3.5" /></button>}<span title="La Bandeja general permanece fija en la raíz" className="mr-1 flex h-9 w-9 items-center justify-center text-slate-300"><LockKeyhole className="h-3 w-3" /></span></div>
}
