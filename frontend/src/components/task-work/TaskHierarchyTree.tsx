...
import { Check, FolderOpen, GripVertical, Info, Loader2, LockKeyhole, MoreHorizontal, Undo2 } from 'lucide-react'
...
function SortableFolderBlock({ folder, items, activeFolderID, activeListID, highlighted, scope, onSelect, onEditFolder, onEditList }: {
  folder: TaskFolder; items: TaskList[]; activeFolderID: string; activeListID: string; highlighted: boolean; scope: HierarchyScope; onSelect: Props['onSelect']; onEditFolder: () => void; onEditList: (list: TaskList) => void
}) {
...
export default function TaskHierarchyTree({ folders, rootLists, scope, collapsed, onSelect, onChanged, onError, onOperation }: Props) {
  ...
  const restore = () => { setOrdered(listSnapshotRef.current); setOrderedFolders(folderSnapshotRef.current); setActiveListID(''); setActiveFolderID(''); setOverContainerID(null) }
  ...
  const handleOver = (event: DragOverEvent) => {
    ...
    setOverContainerID(targetFromOver(event.over)?.containerID || null)
  }
  ...
  return <>
    ...
    {pendingMove && <MoveConfirmation move={pendingMove} folder={folders.find(folder => folder.id === folderIDFromContainer(pendingMove.containerID))!} busy={saving} onConfirm={() => void persistListMove(pendingMove)} onCancel={() => { setOrdered(pendingMove.snapshot); setPendingMove(null) }} />}
