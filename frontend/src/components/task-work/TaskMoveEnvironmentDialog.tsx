...
export default function TaskMoveEnvironmentDialog({ open, task, onClose, onMoved }: Props) {
  ...
  return createPortal(<div data-task-move-environment-dialog className="fixed inset-0 flex items-center justify-center bg-slate-950/55 p-3 backdrop-blur-sm sm:p-6" style={{ zIndex: TASK_OVERLAY_LAYERS.dialog }} onMouseDown={event => { if (event.target === event.currentTarget && !moving) onClose() }}>
