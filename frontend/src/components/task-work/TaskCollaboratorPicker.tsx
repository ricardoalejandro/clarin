...
export default function TaskCollaboratorPicker({ users, value, ownerID, onChange, disabled, pending, emptyLabel = 'Sin colaboradores adicionales.' }: Props) {
  ...
  return <div data-task-collaborator-picker>
    ...
    {open && typeof document !== 'undefined' && createPortal(<>
      <button type="button" aria-label="Cerrar colaboradores" className="fixed inset-0 z-[119] cursor-default" onMouseDown={() => close(false)} />
      <div style={style} className="fixed z-[120] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/15">
        ...
        }} placeholder="Buscar por nombre, usuario o rol…" className="w-full rounded-xl bg-slate-50 py-2.5 pl-10 pr-3 text-sm outline-none focus:bg-white focus:ring-2 focus:ring-emerald-200" /></div>
