...
export default function TaskParticipantGrantConfirmDialog({
  ...
  return createPortal(<div data-task-participant-grant-dialog className="fixed inset-0 flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm" style={{ zIndex: TASK_OVERLAY_LAYERS.confirmation }} onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose() }}>
