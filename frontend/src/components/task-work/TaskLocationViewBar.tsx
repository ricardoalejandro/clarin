...
import type { TaskLocationView, TaskViewMode } from '@/types/task'
import {
...
interface TaskLocationViewBarProps {
  ...
  onCreate: (name: string, operationID: string) => Promise<string | null>
...
function ViewDialog({
  ...
  const description = state.kind === 'trash'
    ...
    : state.kind === 'duplicate'
      ...
      : state.kind === 'rename'
        ? `El nombre cambiará en Work y en el Hub. El acceso seguirá heredándose de ${locationLabel}.`
        : `Se añadirá a ${locationLabel}. El acceso seguirá heredándose de Clarin Work.`
  ...
  return createPortal(<div className="fixed inset-0 flex items-center justify-center bg-slate-950/50 p-3 backdrop-blur-sm sm:p-5" style={{ zIndex: OPERATIONAL_OVERLAY_LAYERS.dialog }} role="presentation" onMouseDown={event => { if (!busy && event.target === event.currentTarget) onClose() }}>
    <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="task-location-view-dialog-title" aria-describedby="task-location-view-dialog-description" className="max-h-[min(720px,calc(100dvh-1.5rem))] w-full max-w-lg overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl outline-none">
      ...
      <div className="p-5">
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" disabled={busy} onClick={onClose} className="min-h-11 rounded-xl px-4 text-sm font-bold text-slate-600 hover:bg-slate-100 disabled:opacity-40">Cancelar</button><button type="button" disabled={busy || ((state.kind === 'create' || state.kind === 'rename') && !name.trim())} onClick={onSubmit} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 text-sm font-black text-white disabled:opacity-40 ${state.kind === 'trash' ? 'bg-rose-600 hover:bg-rose-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}>{busy && <Loader2 className="h-4 w-4 animate-spin" />}{state.kind === 'create' ? 'Añadir pizarra' : state.kind === 'rename' ? 'Guardar nombre' : state.kind === 'duplicate' ? error ? 'Reintentar duplicación' : 'Duplicar pizarra' : 'Mover a Papelera'}</button></div>
      </div>
    </div>
  </div>, document.body)
}
...
export default function TaskLocationViewBar({
  ...
  useEffect(() => {
    ...
    const createRevoked = modal?.kind === 'create' && !canCreate
    ...
    setError('')
    setBusy(false)
  }, [canCreate, featureEnabled, locationContextKey, locationViews, menu?.view.id, modal])
  ...
  const submit = async () => {
    ...
    const failure = modal.kind === 'create'
      ? await onCreate(normalized, modal.operationID)
      : modal.kind === 'rename'
  ...
  return <>
    <div ref={barRef} data-task-view-tabs className="flex min-w-0 flex-1 items-center overflow-hidden rounded-xl bg-slate-100 p-1">
      ...
      <div className="flex min-w-0 items-center gap-1 overflow-hidden">{split.visible.map(view => { const active = activeLocationView?.id === view.id; const hideName = layoutWidth < 520; const menuOpen = menu?.view.id === view.id; return <div key={view.id} className={`group flex min-w-0 shrink items-center rounded-lg ${active ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500 hover:bg-white/70 hover:text-slate-800'}`}><button type="button" onClick={() => onSelectLocation(view)} aria-pressed={active} title={`${view.resource.whiteboard.name} · ${taskLocationViewBreadcrumb(view)}`} className="flex min-h-9 min-w-0 items-center gap-1.5 px-2.5 text-xs font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-500"><PenTool className="h-3.5 w-3.5 shrink-0 text-emerald-600" /><span className={`${hideName ? 'sr-only' : 'max-w-32 truncate'}`}>{view.resource.whiteboard.name}</span></button>{active && view.capabilities.can_manage && <button type="button" onClick={event => { if (menuOpen) closeMenu(); else openMenu(view, event.currentTarget) }} aria-label={`Acciones de ${view.resource.whiteboard.name}`} aria-haspopup="menu" aria-expanded={menuOpen} aria-controls={menuOpen ? 'task-location-view-actions' : undefined} className="flex h-9 w-8 shrink-0 items-center justify-center rounded-r-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700"><MoreHorizontal className="h-3.5 w-3.5" /></button>}</div> })}
