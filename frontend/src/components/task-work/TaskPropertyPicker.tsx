...
function PickerPortal({ label, open, style, highlighted, count, containerRef, onHighlight, onChoose, onClose, children }: { label: string; open: boolean; style: CSSProperties; highlighted: number; count: number; containerRef: React.RefObject<HTMLDivElement>; onHighlight: (index: number) => void; onChoose: (index: number) => void; onClose: () => void; children: React.ReactNode }) {
  ...
  return createPortal(<>
    ...
    <div ref={containerRef} data-task-property-picker-portal role="listbox" aria-label={label} style={{ ...style, zIndex: TASK_OVERLAY_LAYERS.picker }} tabIndex={-1} onKeyDown={event => {
    ...
    }} className="fixed overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-2xl shadow-slate-900/15 outline-none">{children}</div>
  </>, document.body)
}
