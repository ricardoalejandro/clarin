...
function PickerPortal({ label, open, style, highlighted, count, onHighlight, onChoose, onClose, children }: { label: string; open: boolean; style: CSSProperties; highlighted: number; count: number; onHighlight: (index: number) => void; onChoose: (index: number) => void; onClose: () => void; children: React.ReactNode }) {
  if (!open || typeof document === 'undefined') return null
  return createPortal(<>
    <button type="button" aria-label={`Cerrar ${label}`} className="fixed inset-0 z-[119] cursor-default" onMouseDown={onClose} />
    <div role="listbox" aria-label={label} style={style} tabIndex={-1} onKeyDown={event => {
      if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); onClose() }
      if (event.key === 'ArrowDown') { event.preventDefault(); onHighlight(Math.min(count - 1, highlighted + 1)) }
      if (event.key === 'ArrowUp') { event.preventDefault(); onHighlight(Math.max(0, highlighted - 1)) }
      if (event.key === 'Home') { event.preventDefault(); onHighlight(0) }
      if (event.key === 'End') { event.preventDefault(); onHighlight(Math.max(0, count - 1)) }
      if ((event.key === 'Enter' || event.key === ' ') && count) { event.preventDefault(); onChoose(highlighted) }
...
export function TaskStatusPicker({ value, statuses, onChange, placeholder = 'Selecciona un estado', disabled, pending, className = '' }: StatusProps) {
  ...
  return <>
    ...
    <PickerPortal label="Seleccionar estado" open={open} style={style} highlighted={highlighted} count={ordered.length} onHighlight={setHighlighted} onChoose={choose} onClose={close}><div ref={portalRef} tabIndex={-1}>{ordered.map((status, index) => <button key={status.id} type="button" role="option" aria-selected={status.id === value} onMouseEnter={() => setHighlighted(index)} onClick={() => choose(index)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left ${highlighted === index ? 'bg-emerald-50' : 'hover:bg-slate-50'}`}><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-white shadow-sm"><i className="h-3 w-3 rounded-full" style={{ backgroundColor: status.color }} /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-700">{status.name}</span><span className="block text-[10px] font-medium text-slate-400">{categoryLabels[status.category]}</span></span>{status.id === value && <Check className="h-4 w-4 text-emerald-600" />}</button>)}</div></PickerPortal>
  </>
}
