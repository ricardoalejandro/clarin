...
export default function TaskFilterToolbar({ filters, statuses, users, storageScope, scope, view, collapsedStatusIds, groupBy, groupDirection, collapsedGroupKeys, onChange, onApplyView, applyDefaultOnLoad, onDefaultLoadHandled, onError, showChips = true, compact = false, betweenControls }: Props) {
  ...
  return <div className="contents">
    <button ref={filterButtonRef} type="button" onClick={openFilters} aria-label="Filtros" aria-expanded={mode === 'filters'} aria-haspopup="dialog" title={compact ? 'Filtros' : undefined} className={`relative inline-flex h-11 items-center justify-center gap-1.5 rounded-xl border text-xs font-semibold transition ${compact ? 'w-11 px-0' : 'px-3'} ${count ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}><Filter className="h-3.5 w-3.5" />{!compact && 'Filtros'}{count > 0 && <span className={`${compact ? 'absolute -right-1 -top-1' : ''} flex h-5 min-w-5 items-center justify-center rounded-full bg-emerald-600 px-1 text-[9px] font-black text-white`}>{count}</span>}</button>
    {betweenControls}
    <button ref={viewsButtonRef} type="button" onClick={() => setMode(current => current === 'views' ? null : 'views')} aria-label="Vistas guardadas" aria-expanded={mode === 'views'} aria-haspopup="dialog" title={compact ? 'Vistas guardadas' : undefined} className={`inline-flex h-11 items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-600 hover:bg-slate-50 ${compact ? 'w-11 px-0' : 'px-3'}`}><Bookmark className="h-3.5 w-3.5" />{!compact && <span>Vistas</span>}{!compact && <ChevronDown className="h-3 w-3 text-slate-400" />}</button>
