...
export default function SessionObservationPanel({ programId, sessionId, disabled = false, onChanged }: { programId: string; sessionId: string; disabled?: boolean; onChanged?: () => void }) {
  ...
  return <section className="mb-4 rounded-2xl border border-slate-200 bg-slate-50/70 p-3 sm:p-4">
    ...
    <div className="mt-3 flex flex-col gap-2 sm:flex-row"><textarea disabled={disabled} rows={2} maxLength={4000} value={draft} onChange={event => setDraft(event.target.value)} placeholder="Ej.: Se dictó el tema… Observaciones generales…" className="min-h-20 flex-1 resize-y rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 disabled:cursor-not-allowed disabled:opacity-50"/><button type="button" disabled={disabled || saving || !draft.trim()} onClick={() => void create()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white disabled:opacity-50"><Plus className="h-4 w-4"/>Añadir</button></div>
    ...
    {loading ? <div className="flex min-h-20 items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-emerald-600"/></div> : items.length > 0 && <div className="mt-3 space-y-2">{items.map(item => <article key={item.id} className={`rounded-xl border bg-white p-3 ${item.is_pinned ? 'border-amber-200 ring-1 ring-amber-100' : 'border-slate-200'}`}>
      {editing?.id === item.id ? <><textarea disabled={disabled} autoFocus rows={3} maxLength={4000} value={editDraft} onChange={event => setEditDraft(event.target.value)} className="w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-50"/><div className="mt-2 flex justify-end gap-2"><button disabled={disabled} onClick={() => setEditing(null)} className="inline-flex min-h-10 items-center gap-1 rounded-lg px-3 text-xs font-semibold text-slate-600 disabled:opacity-40"><X className="h-4 w-4"/>Cancelar</button><button disabled={disabled || saving || !editDraft.trim()} onClick={() => void update()} className="inline-flex min-h-10 items-center gap-1 rounded-lg bg-emerald-600 px-3 text-xs font-bold text-white disabled:opacity-40"><Save className="h-4 w-4"/>Guardar</button></div></> : <div className="flex gap-3"><div className="min-w-0 flex-1">{item.is_pinned && <span className="mb-1 inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-amber-700"><Pin className="h-3 w-3"/>Fijado</span>}<p className="whitespace-pre-wrap break-words text-sm leading-5 text-slate-700">{item.notes}</p><p className="mt-1 text-[10px] text-slate-400">{item.created_by_name || 'Autor no registrado'} · {new Date(item.created_at).toLocaleString('es-PE')}{item.updated_at !== item.created_at && ` · editado ${new Date(item.updated_at).toLocaleString('es-PE')}`}</p></div><div className="flex shrink-0 gap-1">{item.can_pin && <button disabled={disabled} onClick={() => void pin(item)} title={item.is_pinned ? 'Desfijar' : 'Fijar'} className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-400 hover:bg-amber-50 hover:text-amber-700 disabled:opacity-40"><Pin className="h-4 w-4"/></button>}{item.can_edit && <button disabled={disabled} onClick={() => { setEditing(item); setEditDraft(item.notes) }} title="Editar" className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 disabled:opacity-40"><Edit2 className="h-4 w-4"/></button>}{item.can_delete && <button disabled={disabled} onClick={() => void remove(item)} title="Eliminar" className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 disabled:opacity-40"><Trash2 className="h-4 w-4"/></button>}</div></div>}
    </article>)}</div>}
  </section>
}
