...
const entry = path.join(sourceRoot, 'offline-v3', 'entry.tsx')
const recoveryEntry = path.join(sourceRoot, 'offline-v3', 'recovery.ts')
const outputRoot = path.join(root, 'public', 'offline-v3')
  ...
  for (const item of entries) {
    ...
    if (item.isDirectory()) await assertNoNextRuntimeImports(absolute)
      ...
      if (/\b(?:from|import\s*)\s*\(?\s*['"]next(?:\/|['"])/.test(source)) {
        throw new Error(`offline-v3 cannot import Next runtime: ${path.relative(root, absolute)}`)
      }
    }
  }
}
...
await Promise.all([
  assertNoNextRuntimeImports(path.join(sourceRoot, 'offline-v3')),
  assertNoNextRuntimeImports(path.join(sourceRoot, 'components', 'offline-v3')),
])

if (outputRoot !== path.join(root, 'public', 'offline-v3')) throw new Error('refusing to clean an unexpected offline-v3 output path')
...
for (const input of Object.keys(result.metafile.inputs)) {
  ...
  if (normalized.includes('/node_modules/next/') || /(?:^|\/)next(?:\/|$)/.test(normalized)) {
    throw new Error(`offline-v3 transitively imported the Next runtime: ${input}`)
  }
}
...
if (!javascript || !recoveryJavascript || !css) throw new Error('offline-v3 build did not produce its JavaScript, recovery and CSS assets')
...
<head>
  ...
  <meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data: blob:; connect-src 'self' http://127.0.0.1:17373; font-src 'self' data:; worker-src 'self'; base-uri 'none'; form-action 'self'">
...
const artifactPaths = [path.join(outputRoot, 'index.html'), ...outputEntries.map(([filename]) => path.resolve(root, filename)), ...await generatedFiles(whiteboardOutput)]
const assets = []
...
await fs.writeFile(path.join(outputRoot, 'manifest.json'), `${JSON.stringify({
  ...
  shell: '/offline-v3/index.html',
  assets,
}, null, 2)}\n`)

const totalBytes = assets.reduce((sum, asset) => sum + asset.bytes, 0)
if (totalBytes > 32 * 1024 * 1024) throw new Error(`offline-v3 public shell exceeds 32 MiB (${totalBytes} bytes)`)
process.stdout.write(`offline-v3 shell ${buildID}: ${assets.length} assets, ${totalBytes} bytes\n`)
