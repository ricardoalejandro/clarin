...
export class ExcalidrawFontFace {
  ...
  constructor(family: string, uri: string, descriptors?: FontFaceDescriptors) {
    ...
    this.sources = this.urls
      .map((url) => `url(${url}) ${ExcalidrawFontFace.getFormat(url)}`)
      .join(", ");

    this.fontFace = this.createFontFace();
  }

  private createFontFace() {
    return new FontFace(this.family, this.sources, {
      display: "swap",
      style: "normal",
      weight: "400",
      ...this.descriptors,
  ...
  public resetAfterLoadFailure() {
    const failedFontFace = this.fontFace;
    this.fontFace = this.createFontFace();
    return failedFontFace;
  }
