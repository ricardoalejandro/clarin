...
} from "./clarinFontPreload";

export interface ClarinFontPreloadProgress {
...
const EMPTY_CLARIN_FONT_PRELOAD_PROGRESS: ClarinFontPreloadProgress = {
  phase: "idle",
  total: 0,
  loaded: 0,
  failed: 0,
...
export class Fonts {
  ...
  private static clarinPreloadProgress: ClarinFontPreloadProgress =
    EMPTY_CLARIN_FONT_PRELOAD_PROGRESS;
  private static readonly clarinPreloadListeners = new Set<
    (progress: ClarinFontPreloadProgress) => void
  >();
  private static readonly clarinReadyFamilies = new Set<number>();
  private static readonly clarinFailedFamilies = new Set<number>();
  private static clarinPreloadPromise: Promise<ClarinFontPreloadProgress> | null =
    null;
  private static clarinOnlineRetryBound = false;
  private static clarinLastInstance: Fonts | null = null;
  ...
  constructor(scene: Scene) {
    this.scene = scene;
    Fonts.clarinLastInstance = this;
    if (!Fonts.clarinOnlineRetryBound && typeof window !== "undefined") {
      Fonts.clarinOnlineRetryBound = true;
      window.addEventListener("online", () => {
        if (
          Fonts.clarinPreloadProgress.phase === "offline" ||
          Fonts.clarinFailedFamilies.size
        ) {
          void Fonts.clarinLastInstance?.preloadSelectableFonts(true);
        }
      });
    }
  }

  private static publishClarinPreloadProgress(
    phase: ClarinFontPreloadProgress["phase"],
    total: number,
  ) {
    Fonts.clarinPreloadProgress = {
      phase,
      total,
      loaded: Fonts.clarinReadyFamilies.size,
      failed: Fonts.clarinFailedFamilies.size,
      readyFamilyIds: Array.from(Fonts.clarinReadyFamilies).sort(
        (left, right) => left - right,
      ),
      failedFamilyIds: Array.from(Fonts.clarinFailedFamilies).sort(
        (left, right) => left - right,
      ),
    };
    Fonts.clarinPreloadListeners.forEach((listener) =>
      listener(Fonts.clarinPreloadProgress),
    );
  }

  public static getPreloadProgress = () => Fonts.clarinPreloadProgress;

  public static subscribePreloadProgress = (
    listener: (progress: ClarinFontPreloadProgress) => void,
  ) => {
    Fonts.clarinPreloadListeners.add(listener);
    listener(Fonts.clarinPreloadProgress);
    return () => {
      Fonts.clarinPreloadListeners.delete(listener);
    };
  };

  public subscribePreloadProgress = Fonts.subscribePreloadProgress;
  public getPreloadProgress = Fonts.getPreloadProgress;

  /** Loads every selectable local family before the catalog is interactive. */
  public preloadSelectableFonts = async (retryFailed = false) => {
    if (Fonts.clarinPreloadPromise) {
      return Fonts.clarinPreloadPromise;
    }
    const selectable = Array.from(Fonts.registered.entries()).filter(
      ([, { metadata }]) => !metadata.serverSide && !metadata.fallback,
    );
    const total = selectable.length;
    const sceneFamilies = new Set(this.getSceneFamilies());
    const ordered = selectable.sort(([left], [right]) => {
      const leftPriority = sceneFamilies.has(left) ? 0 : 1;
      const rightPriority = sceneFamilies.has(right) ? 0 : 1;
      return leftPriority - rightPriority || left - right;
    });
    if (typeof navigator !== "undefined" && !navigator.onLine) {
      Fonts.publishClarinPreloadProgress("offline", total);
      return Fonts.clarinPreloadProgress;
    }
    const targetFamilies = new Set(
      selectClarinFontPreloadTargets({
        orderedFamilyIds: ordered.map(([family]) => family),
        readyFamilyIds: Fonts.clarinReadyFamilies,
        failedFamilyIds: Fonts.clarinFailedFamilies,
        retryFailed,
      }),
    );
    const targets = ordered.filter(([family]) => targetFamilies.has(family));
    if (!targets.length) {
      Fonts.publishClarinPreloadProgress(
        Fonts.clarinFailedFamilies.size ? "error" : "complete",
        total,
      );
      return Fonts.clarinPreloadProgress;
    }
    Fonts.publishClarinPreloadProgress("loading", total);
    Fonts.clarinPreloadPromise = (async () => {
      let cursor = 0;
      const loadedFaces: FontFace[] = [];
      const worker = async () => {
        while (cursor < targets.length) {
          const targetIndex = cursor++;
          const [family, registration] = targets[targetIndex];
          try {
            if (registration.metadata.local) {
              Fonts.clarinReadyFamilies.add(family);
              Fonts.clarinFailedFamilies.delete(family);
              Fonts.publishClarinPreloadProgress("loading", total);
              continue;
            }
            const faces = await Promise.all(
              registration.fontFaces.map(async (registeredFace) => {
                let { fontFace } = registeredFace;
                if (shouldResetClarinFontFace({
                  retryFailed,
                  familyFailed: Fonts.clarinFailedFamilies.has(family),
                  status: fontFace.status,
                })) {
                  window.document.fonts.delete(fontFace);
                  registeredFace.resetAfterLoadFailure();
                  fontFace = registeredFace.fontFace;
                }
            ...
            Fonts.clarinFailedFamilies.add(family);
            console.error(`No se pudo precargar la fuente ${family}.`, error);
          }
          Fonts.publishClarinPreloadProgress("loading", total);
        }
      };
      await Promise.all(
        Array.from(
          { length: Math.min(4, Math.max(1, targets.length)) },
          () => worker(),
        ),
      );
      if (loadedFaces.length) {
        this.onLoaded(loadedFaces);
      }
      ...
      Fonts.clarinPreloadPromise = null;
    });
    return Fonts.clarinPreloadPromise;
  };
