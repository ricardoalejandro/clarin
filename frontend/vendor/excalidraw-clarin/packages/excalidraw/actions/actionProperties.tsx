...
import {
  CLARIN_TEXT_MARK,
  getActiveClarinRichTextEditor,
  getClarinMarkState,
  getClarinRichTextStateVersion,
  getClarinTextFormat,
  setClarinTextMark,
  shouldForceClarinCustomDataRemoval,
  subscribeClarinRichTextState,
  updateClarinTextCustomData,
  type ClarinTextMark,
  type ClarinTextMarkState,
...
const CLARIN_TEXT_MARK_OPTIONS = [
  {
    mark: CLARIN_TEXT_MARK.BOLD,
    label: "Negrita",
    shortcut: "Ctrl/Cmd+B",
    icon: <strong aria-hidden="true">B</strong>,
  },
  {
    mark: CLARIN_TEXT_MARK.ITALIC,
    label: "Cursiva",
    shortcut: "Ctrl/Cmd+I",
    icon: <em aria-hidden="true">I</em>,
  },
  {
    mark: CLARIN_TEXT_MARK.UNDERLINE,
    label: "Subrayado",
    shortcut: "Ctrl/Cmd+U",
    icon: <u aria-hidden="true">U</u>,
  },
  {
    mark: CLARIN_TEXT_MARK.STRIKE,
    label: "Tachado",
    shortcut: "Ctrl/Cmd+Shift+X",
    icon: <s aria-hidden="true">S</s>,
  },
] as const;

const getClarinTextTargets = (
...
const aggregateClarinMarkState = (
  elements: readonly ExcalidrawElement[],
  appState: AppState,
  mark: ClarinTextMark,
): ClarinTextMarkState => {
  const activeEditor = getActiveClarinRichTextEditor();
  if (
    activeEditor &&
    activeEditor.elementId === appState.editingTextElement?.id
  ) {
    return activeEditor.getMarkState(mark);
  }
  const targets = getClarinTextTargets(elements, appState);
  if (!targets.length) {
    return "off";
  }
  const states = targets.map((element) =>
    getClarinMarkState(
      getClarinTextFormat(element),
  ...
  return states.every((state) => state === "on")
    ? "on"
...
const RichTextMarksPanel = ({
  ...
  updateData: (mark: ClarinTextMark) => void;
}) => {
  useSyncExternalStore(
    subscribeClarinRichTextState,
    getClarinRichTextStateVersion,
  ...
  if (!appProps.enableRichText) {
    return null;
  }
  const hasTarget = Boolean(
    appState.editingTextElement || getClarinTextTargets(elements, appState).length,
  );
  return (
    <fieldset className="clarin-rich-text-marks">
      <legend>Estilo de texto</legend>
      <div className="buttonList">
        {CLARIN_TEXT_MARK_OPTIONS.map((option) => {
          const state = aggregateClarinMarkState(
            elements,
            appState,
            option.mark,
          );
          return (
            <ButtonIcon
              ...
              testId={`clarin-text-mark-${option.label.toLowerCase()}`}
              className={`properties-trigger clarin-rich-text-mark${
                state === "mixed" ? " clarin-rich-text-mark--mixed" : ""
              }`}
              active={state !== "off"}
              ariaPressed={state === "mixed" ? "mixed" : state === "on"}
              disabled={!hasTarget}
              onPointerDown={(event) => {
                const activeEditor = getActiveClarinRichTextEditor();
                if (
              ...
              onClick={() => updateData(option.mark)}
            />
          );
        })}
      </div>
...
const performClarinTextMark = (
  defaultMark: ClarinTextMark,
  elements: readonly ExcalidrawElement[],
  appState: AppState,
  value: unknown,
  app: AppClassProperties,
) => {
  const mark = CLARIN_TEXT_MARK_OPTIONS.some((option) => option.mark === value)
    ? (value as ClarinTextMark)
    : defaultMark;
  const activeEditor = getActiveClarinRichTextEditor();
  if (
    activeEditor &&
  ...
  const shouldApply = !targets.length
    ? false
    : !targets.every(
        (element) =>
          getClarinMarkState(
  ...
  const nextElements = elements.map((element) => {
    if (!targetIDs.has(element.id) || !isTextElement(element)) {
      return element;
    }
    const format = setClarinTextMark(
      getClarinTextFormat(element),
      element.originalText,
      0,
      element.originalText.length,
    ...
    const nextCustomData = updateClarinTextCustomData(
      element.customData,
      format,
    );
...
export const actionToggleTextBold = register({
  name: "toggleTextBold",
  label: "Negrita",
  trackEvent: false,
  perform: (elements, appState, value, app) =>
    performClarinTextMark(
  ...
  keyTest: (event) => clarinMarkShortcut(event, "b"),
  PanelComponent: RichTextMarksPanel,
});

export const actionToggleTextItalic = register({
  name: "toggleTextItalic",
  label: "Cursiva",
  trackEvent: false,
  perform: (elements, appState, value, app) =>
    performClarinTextMark(
  ...
  keyTest: (event) => clarinMarkShortcut(event, "i"),
});

export const actionToggleTextUnderline = register({
  name: "toggleTextUnderline",
  label: "Subrayado",
  trackEvent: false,
  perform: (elements, appState, value, app) =>
    performClarinTextMark(
  ...
  keyTest: (event) => clarinMarkShortcut(event, "u"),
});

export const actionToggleTextStrike = register({
  name: "toggleTextStrike",
  label: "Tachado",
  trackEvent: false,
  perform: (elements, appState, value, app) =>
    performClarinTextMark(
  ...
  keyTest: (event) => clarinMarkShortcut(event, "x", true),
});
...
export const actionChangeTextAlign = register({
  ...
  perform: (elements, appState, value, app) => {
    ...
    if (
      ...
      activeEditor.setParagraphAlignment(value as TextAlign);
      return {
        captureUpdate: CaptureUpdateAction.NEVER,
      };
    }
    return {
      elements: changeProperty(
        ...
        (oldElement) => {
          if (isTextElement(oldElement)) {
            const nextCustomData = updateClarinParagraphCustomData(
  ...
  PanelComponent: ({ elements, appState, updateData, app }) => {
    ...
    const activeEditor = getActiveClarinRichTextEditor();
    const isEditingActiveText = Boolean(
      activeEditor &&
        activeEditor.elementId === appState.editingTextElement?.id,
    );
    const targets = getClarinTextTargets(elements, appState);
    const targetStates = targets.map((target) =>
      getClarinParagraphAlignmentState(
        getClarinParagraphFormat(target),
        target.originalText,
        target.textAlign,
        0,
        target.originalText.length,
      ),
    );
    const activeState = isEditingActiveText
      ? activeEditor!.getParagraphAlignmentState()
      : null;
    const paragraphAlignmentValue: TextAlign | false | null = activeState
      ? activeState === "mixed"
        ? false
        : activeState
      : targetStates.length
      ? targetStates.every(
          (state) => state !== "mixed" && state === targetStates[0],
        )
        ? (targetStates[0] as TextAlign)
        : false
    ...
    return (
      ...
      >
        ...
        <span className="visually-hidden" aria-live="polite">
          {paragraphAlignmentValue === false
            ? "Alineación mixta"
            : paragraphAlignmentValue
            ? `Alineación ${paragraphAlignmentValue}`
            : "Sin alineación seleccionada"}
        </span>
