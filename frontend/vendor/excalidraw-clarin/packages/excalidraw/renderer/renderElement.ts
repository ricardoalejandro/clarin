...
import type { FontString } from "../element/types";
import { getClarinVisualLineLayouts } from "./clarinParagraphAlignment";
  ...
  switch (element.type) {
    ...
    default: {
      if (isTextElement(element)) {
        ...
        const visualLines = getClarinVisualLines(element);
        const visualLineLayouts = getClarinVisualLineLayouts(element);
        const hasRichText = Boolean(getClarinTextFormat(element)?.runs.length);
        ...
        for (let index = 0; index < lines.length; index++) {
          ...
          const lineTextAlign =
            visualLineLayouts[index]?.textAlign || element.textAlign;
          const horizontalOffset =
            lineTextAlign === "center"
              ...
              : 0;
          if (!hasRichText) {
            context.textAlign = lineTextAlign as CanvasTextAlign;
            context.fillText(lines[index], horizontalOffset, baseline);
            continue;
          }
          const runs = visualLines[index] || [];
          const runWidths = runs.map((run) =>
            getLineWidth(
              run.text,
              getClarinFontForMarks(baseFont, run.marks) as FontString,
            ),
          );
          ...
          const orderedWidths = rtl ? [...runWidths].reverse() : runWidths;
          context.textAlign = rtl ? "right" : "left";
          for (let runIndex = 0; runIndex < orderedRuns.length; runIndex++) {
            const run = orderedRuns[runIndex];
            const runWidth = orderedWidths[runIndex];
            context.font = getClarinFontForMarks(
              baseFont,
              run.marks,
            ) as FontString;
            context.fillText(
              run.text,
              rtl ? runX + runWidth : runX,
              baseline,
            );
            const thickness = Math.max(1, element.fontSize / 16);
            if (run.marks & CLARIN_TEXT_MARK.UNDERLINE) {
              context.fillRect(
                runX,
                baseline + element.fontSize * 0.08,
                runWidth,
                thickness,
              );
            }
            if (run.marks & CLARIN_TEXT_MARK.STRIKE) {
              context.fillRect(
                runX,
