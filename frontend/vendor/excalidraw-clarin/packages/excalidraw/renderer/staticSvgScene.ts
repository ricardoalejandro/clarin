    ...
    default: {
      if (isTextElement(element)) {
        ...
        for (let i = 0; i < lines.length; i++) {
          const lineTextAlign =
            visualLineLayouts[i]?.textAlign || element.textAlign;
          const horizontalOffset =
            lineTextAlign === "center"
              ...
              ? element.width
              : 0;
          const textAnchor = getClarinSvgTextAnchor(
            lineTextAlign,
            direction,
          );
          const text = svgRoot.ownerDocument!.createElementNS(SVG_NS, "text");
          if (hasRichText) {
            for (const run of visualLines[i] || []) {
              const tspan = svgRoot.ownerDocument!.createElementNS(
                SVG_NS,
                "tspan",
              );
              tspan.textContent = run.text;
              if (run.marks & CLARIN_TEXT_MARK.BOLD) {
                tspan.setAttribute("font-weight", "700");
              }
              if (run.marks & CLARIN_TEXT_MARK.ITALIC) {
                tspan.setAttribute("font-style", "italic");
              }
              const decoration = getClarinTextDecoration(run.marks);
              if (decoration) {
