...
export const FontPicker = React.memo(
  ({
    ...
    const preloadProgress = useSyncExternalStore(
      Fonts.subscribePreloadProgress,
      Fonts.getPreloadProgress,
      Fonts.getPreloadProgress,
    );
    const selectorPending =
      preloadProgress.phase === "idle" ||
      preloadProgress.phase === "loading" ||
      preloadProgress.phase === "offline";
    const failedFamilies = new Set(preloadProgress.failedFamilyIds);
    ...
    return (
      <div role="dialog" aria-modal="true" className="FontPicker__container">
        <ButtonIconSelect<FontFamilyValues | false>
          type="button"
          options={defaultFonts.map((font) => ({
            ...font,
            disabled: selectorPending || failedFamilies.has(font.value),
          }))}
        ...
        <ButtonSeparator />
        <Popover.Root
          open={isOpened && !selectorPending}
          onOpenChange={(open) => onPopupChange(selectorPending ? false : open)}
        >
          <FontPickerTrigger
            selectedFontFamily={selectedFontFamily}
            disabled={selectorPending}
          />
          {isOpened && (
            <FontPickerList
              selectedFontFamily={selectedFontFamily}
