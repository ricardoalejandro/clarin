...
- Count participant attendance only in `enrolled_at <= session.date < earliest(dropped_at, completed_at)`: enrollment is inclusive and withdrawal/completion are exclusive first-out days. Return real out-of-window attendance separately as history and distinguish "no sessions in the period" from "no attendance exists".
