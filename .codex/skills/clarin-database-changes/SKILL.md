...
- Attendance eligibility uses calendar dates in `enrolled_at <= session.date < earliest(dropped_at, completed_at)`. Enrollment is inclusive; withdrawal/completion are exclusive first-out dates. Keep recorded attendance outside that range as historical data.
