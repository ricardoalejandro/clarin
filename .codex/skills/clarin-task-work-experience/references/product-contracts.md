...
- Ordinary tasks may have no dates, a due date only, a start date only, or an explicit start/due range. Creation presents the optional due date first and keeps start behind progressive disclosure; never derive or persist `start_at` from `created_at` or the current day without an explicit user choice. Calendar slot creation remains the exception because the selected slot supplies a real interval that “More options” must preserve exactly.
...
- Status and priority property controls must use accessible, viewport-aware portaled pickers with real status/category and priority semantics. They support pointer and keyboard operation and expose saving, conflict, rollback, and retry behavior. Priority is always visible as its own flag/text semantic in task views; status is also visible unless the current column, group, or control already communicates that same status. Neither semantic may reuse the task identity color.
...
- Task detail docked mode is an in-flow complementary inspector, not an overlay: it has no veil or blur (`0`/`none`), remains between 440–672 px and preserves at least 480 px for the measured Work surface. When the measured space after Work navigation and Eros is below 920 px, it becomes temporarily maximized without overwriting the remembered mode. Floating remains non-modal with an 18% veil and 2 px blur; maximized/mobile remains modal with 45% and 3 px.
- While the docked or floating detail is open, every task view stays navigable. A click, Enter, Space, or short touch on another task updates the mounted inspector without resetting its mode, width, Details/Activity view, workspace filters, or workspace scroll. The open-task signal is neutral, uses `aria-current`, and remains distinct from multi-selection.
- Detail reads are isolated by task ID and generation and abort when navigation changes the active task. Writes already sent still reconcile their own task, but their pending, error, retry, or response state may never appear under another task. Title, description, comment/mentions/attachments, comment editing, and quick-subtask drafts are retained independently per task for the mounted Work session and are cleared only after their own successful save or explicit discard.
