...
- Verify docked, floating, resized, maximized, mobile, Escape, focus restoration, and workspace interaction behind non-modal modes. Assert detail-specific `0`/`none` for the in-flow docked inspector, 18%/2 px for floating and 45%/3 px for maximized/mobile, with modal blocking only for maximized/mobile. At measured widths 1440, 1280, 1024, 768 and 375 px plus Eros/sidebar combinations, prove docked detail remains 440–672 px, preserves 480 px of Work, and temporarily maximizes below 920 px without changing the remembered preference.
- Open A and navigate directly to B/C behind the mounted inspector in Lista, Tablero and Gantt; in Calendario preserve the first summary but navigate directly once detail is already open. Prove geometry, tab, filters and workspace scroll remain stable; the neutral `aria-current` signal follows the open task independently of multi-selection.
- Invert the completion order of all six reads and representative writes for A/B. Prove no title/section/error/pending state from A renders in B, A still reconciles in every view, and returning to either task restores only its own title, description, comment/mentions/attachments, comment-edit and quick-subtask drafts. Verify successful save and explicit discard clear only the intended task draft.
- Render low, medium, high and urgent priority in Lista, Tablero, Calendario and Gantt before and after HTTP, WebSocket, close/reopen and reload. Render real workflow status where it is not already represented by grouping/column, never duplicate it where it is, and prove neither indicator changes `task.color`/`resolved_color`. Exercise same-version echoes, newer canonical versions, `409`, transport failure and out-of-order responses; rollback only the still-active `operation_id`.
